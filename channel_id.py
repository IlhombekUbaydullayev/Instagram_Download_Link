...
# Aiogram bot (faqat tekshiruv uchun ishlatiladi)
aiobot = AioBot(token=BOT_TOKEN)


# 🔐 A'zolikni tekshirish funksiyasi
async def check_user_in_channel(bot: AioBot, user_id: int, channel_username: str) -> bool:
    try:
...
async def download_handler(client: Client, message: Message):
    ...
    is_member = await check_user_in_channel(aiobot, message.from_user.id, CHANNEL_USERNAME)
    if not is_member:
        join_button = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔔 Kanalga a'zo bo‘lish", url=CHANNEL_LINK)]]
        )
        return await message.reply(
            "❗ Videoni olish uchun avval kanalga a'zo bo‘ling:",
    ...
    try:
        ...
        if video:
            ...
            try:
                ...
                await message.reply(f"🔗 {CHANNEL_LINK}")
        else:
            await wait_msg.edit("❌ Video yuklab bo‘lmadi. Havola noto‘g‘ri yoki format qo‘llab-quvvatlanmaydi.")
    except Exception as e:
        await wait_msg.edit(f"❌ Xatolik: {e}")
